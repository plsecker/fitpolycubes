#!/usr/bin/env python3
"""
Corrected complete 4×5 Macro state-graph analysis for S-pentacube.

Uses fwd/bwd intersection instead of Tarjan for correctness.
"""
...
# ============================================================================
# Correct SCC computation via fwd/bwd intersection
# ============================================================================
...
def compute_sccs(succ, all_nodes):
    ...
    # Second pass: explore reverse graph in order of decreasing finish time
    visited2 = set()
    sccs = []
...
def find_cycles_in_scc(scc_nodes, succ):
        ...
        while stack:
            ...
            for nxt in adj.get(node, []):
                if nxt == start and len(path) > 1:
                    # Check if this is a canonical rotation
                    # (min element first)
                    canon = tuple(path)
                    cycles.append(canon)
    ...
    # Deduplicate: cycles may be found multiple times
    seen_cycles = set()
    unique_cycles = []
    for c in cycles:
        # Normalize by rotating to start at the minimum element
        m = min(c)
        idx = c.index(m)
        normalized = c[idx:] + c[1:idx+1]  # rotate so m is first
...
def main():
    print("=" * 70)
    print("COMPLETE 4×5 S-PENTACUBE MACRO STATE GRAPH ANALYSIS")
    print("=" * 70)
    print()
    ...
    # Compute all distances from 0 (forward reachability)
    distances = {0: {0}}
    queue = deque([0])
    processed = set()
    while queue:
    ...
    # Tileable N
    print("--- Tileable N ---")
    # The graph is finite, so we can compute all reachable distances
    # For each state, we have the set of distances from 0
    # N is tileable iff N ∈ distances[0]
    ...
    # States
    with open(data_dir / "s_4x5_states.txt", "w") as f:
        for s in state_list:
            f.write(f"{s}\n")
    print(f"  States: {data_dir / 's_4x5_states.txt'}")
    
    # Edges
    with open(data_dir / "s_4x5_edges.txt", "w") as f:
        for src in sorted(succ.keys()):
            dsts = sorted(succ[src])
            f.write(f"{src} -> {' '.join(str(d) for d in dsts)}\n")
    print(f"  Edges: {data_dir / 's_4x5_edges.txt'}")
    
    # SCCs
    with open(data_dir / "s_4x5_sccs.txt", "w") as f:
        for i, scc in enumerate(sccs):
            scc_list = sorted(scc)
            internal_edges = sum(1 for s in scc for t in succ.get(s, []) if t in scc)
            outgoing_edges = sum(1 for s in scc for t in succ.get(s, []) if t not in scc)
            ...
            for s in scc_list:
                f.write(f"  {s}\n")
    print(f"  SCCs: {data_dir / 's_4x5_sccs.txt'}")
    
    # Cycles
    with open(data_dir / "s_4x5_cycles.txt", "w") as f:
    ...
    print(f"  Cycles: {data_dir / 's_4x5_cycles.txt'}")
    
    # Returns
    with open(data_dir / "s_4x5_returns.txt", "w") as f:
    ...
    print(f"  Returns: {data_dir / 's_4x5_returns.txt'}")
    ...
    with open(data_dir / "s_4x5_summary.json", "w") as f:
        json.dump(summary, f, indent=2)
    print(f"  Summary: {data_dir / 's_4x5_summary.json'}")
