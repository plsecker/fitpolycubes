...
- **2026-09-01**: Applied the **thickness-1 published-family promotion** (derived theorem; see `docs/frontier/y_piece/y_thickness1_published_families_patch.md` and `docs/frontier/post_promotion_integrity_report.md`). Added a `searched_no_solution` precedence guard (classify consults `impossible_reason` first; all 48 searched boxes keep their labels) and three families from the published qu5-y rectangle classification: `1×5×N` with N ≢ 0 (mod 10), `1×6×N`, `1×8×N` — each a **derived theorem with published ingredients** (Golomb JoCT 1966; Bouwkamp–Klarner 1970; Sillke qu5-y). 6 boxes (dims ≤ 20) moved UNKNOWN → impossible; the solver's Y 1×5×10 count (10) matches the published 5×10 rectangle as sanity, not proof. Side effect (intended): all 48 searched boxes now carry the `searched_no_solution` label through `classify`; the search database's unproved count dropped 199 → 160 (exactly the 39 previously-unlabeled Y searched entries).
