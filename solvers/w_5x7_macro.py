#!/usr/bin/env python3
"""
Macro state-graph solver for W pentacube in 5×7×N boxes.

This implements the generalized Macro technique for the W pentacube
with 5×7 cross-section, targeting the minimal box 5×7×9.

State representation:
    One Python integer containing three 35-bit z-layer masks.

    bits   0..34   = layer 0
    bits  35..69   = layer 1
    bits  70..104  = layer 2

The W pentacube spans at most 3 layers in any placement, so we need
exactly 3 layers to capture the frontier state.

Usage:
    # Build state graph and count paths
    python3 solvers/w_5x7_macro.py --target-depth 9

    # With checkpointing
    python3 solvers/w_5x7_macro.py --target-depth 9 \
        --checkpoint /tmp/w_5x7.ckpt

    # Enumerate actual tilings
    python3 solvers/w_5x7_macro.py --target-depth 9 --enumerate-tilings
"""
...
X_SIZE = 5
Y_SIZE = 7
NCELLS = X_SIZE * Y_SIZE  # 35
LAYERS = 3
WORD_MASK = (1 << NCELLS) - 1  # 35 bits
...
def make_shifted_template(
    ...
    """
    Translate a concrete W placement so that the chosen target occurrence
    lies on frontier layer 0.

    Return packed 105-bit occupancy (three 35-bit layer masks).
    """
...
def build_templates() -> Tuple[Dict[int, List[int]], int, int, int, int]:
    """
    Generate templates for W pentacube in 5×7 cross-section.
    ...
    Returns:
        ...
        total_templates: number of templates generated
    """
...
def build_macro_graph(
    ...
    """
    Build the complete Macro state graph for W in 5×7×N.
...
def main():
    parser = argparse.ArgumentParser(
        description="Macro state-graph solver for W pentacube in 5×7×N boxes."
    )
    ...
    if verbose:
        print("=" * 70)
        print("W Pentacube Macro Solver")
    ...
    if args.output:
        ...
        output_path = Path("/tmp/w_5x7x9_results.json")
        with open(output_path, "w") as f:
            json.dump(results, f, indent=2)
        if verbose:
            print(f"\nResults written to {output_path}")
