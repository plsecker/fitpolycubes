#!/usr/bin/env python3
"""
Finite-box diagonal frontier DP for S in 4x8xN.

Correct frontier-state invariant:

    masks[0..4] contain ONLY cells occupied by pieces that still reach
    the unprocessed side of the diagonal cut.

Resolved cells behind the cut are erased from the state.

To preserve correctness after erasing history:
    * a candidate placement may not touch any plane d < current_d;
    * on current_d it may not touch any cell before the cursor.

This is the first version that explicitly represents a true exposed
frontier rather than the full history of the partial tiling.

The state is:
    (current absolute diagonal d,
     cursor within that diagonal,
     five occupancy bitsets for d..d+4)

Memoization therefore has a chance to merge different histories that leave
the same exposed ragged boundary.
"""
...
def build_plane_order(
    plane_shapes,
):
    """
    Deterministic order of cells within each solid-diagonal plane.
    """
    order = {}
...
def build_processed_masks(
    plane_order,
):
    """
    processed_masks[d][cursor] is the bitset of cells that lie before
    cursor on diagonal d.
    """
    result = {}
...
def build_placements(
    ...
    plane_shapes = build_plane_shapes(
        X, Y, Z
    )
...
def normalize_state(
    ...
    """
    Move the cursor over occupied cells.

    Crucially, cells behind the cursor are erased from the state.
    """
    while True:
        order = plane_order[d]

        # Forget every occupied cell before the current cursor.
        if cursor > 0:
            forgotten_mask = 0

            for bit in order[:cursor]:
                bit_mask = 1 << bit
                if masks[0] & bit_mask:
                    forgotten_mask |= bit_mask
                    masks[0] &= ~bit_mask

            stats.forgotten_cells += forgotten_mask.bit_count()
        ...
        # Current diagonal is fully resolved.
        d += 1
        cursor = 0
...
def placement_fits_frontier(
    ...
    """
    Check:
      1. placement is entirely on/after the cut;
      2. it does not overlap exposed frontier occupancy;
      3. it does not reach back into already-resolved cells of current_d.
    """
    for sl in placement.slices:
        ...
        # On the current diagonal, bits before cursor are behind the cut.
        if rel == 0:
            processed = processed_masks[
                current_d
            ][cursor]
        ...
        # For rel >= 0, check collision only with surviving frontier bits.
...
class FrontierSolver:
    ...
    def solve(
        ...
        # Normalize / forget history BEFORE memoization.
        ...
        for placement_index in self.by_cell.get(
            cell,
            (),
        ):
    ...
    def run(self) -> bool:
        ...
        print(
            f"Forgotten frontier cells: "
            f"{self.stats.forgotten_cells:,}"
        )
        print(
            f"Solutions: "
...
def main() -> int:
    ...
    plane_order = build_plane_order(
        plane_shapes
    )

    processed_masks = build_processed_masks(
        plane_order
    )
    ...
    print(
        "Searching corrected exposed-frontier DP...",
        flush=True,
    )
