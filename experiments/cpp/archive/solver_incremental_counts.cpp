    ...
    SearchStats& stats)
{
    ...
    deactivated_rows.reserve(256);
    decremented_cols.reserve(512);
    ...
    for (int k = row_start; k < row_end; ++k) {
        ...
        for (int c : decremented_cols) {
            ++candidate_counts[c];
        }

        for (int other : deactivated_rows) {
            active_rows[other] = 1;
        }
